...
def henon_heiles(v):
    """
    The Henon-Heiles system Hamiltonian
    H = 1/2 * (p_x^2 + x^2) + 1/2 * (p_y^2 + y^2) + lambda * (x^2 y - 1/3 y^3)
    :param v:
    :return:
    """
    ...
    Hc += QuadOperator('q0 q0 q1', v[2])
    Hc += QuadOperator('q1 q1 q1', v[3])

    return H_real(normal_ordered(get_boson_operator(Hc)))


def couplied_harmonic_oscillator(v):
    """
    The coupled Harmonic oscillator Hamiltonian proposed by https://arxiv.org/pdf/1710.01158
    :return:
    """
    Hc = QuadOperator('p0 p0', v[0])
    Hc += QuadOperator('p1 p1', v[1])
    Hc += QuadOperator('q0 q0', v[2])
    Hc += QuadOperator('q1 q1', v[3])
    Hc += QuadOperator('q0 q1', v[4])
    return H_real(normal_ordered(get_boson_operator(Hc)))
...
if __name__ == '__main__':
    ...
    h_variables = [1 / 2, 1 / 2, 1, -1/3]
    hh_hamil = henon_heiles(h_variables)
    print(type(hh_hamil))
